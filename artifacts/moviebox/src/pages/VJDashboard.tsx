...
const initMovies: Movie[] = [
  { id: 1, title: "Deadpool & Wolverine", category: "Action", year: 2024, url: "https://example.com/dp.mp4", popular: true, thumbnail: "", views: 12400 },
  { id: 2, title: "Dune: Part Two", category: "Sci-Fi", year: 2024, url: "https://example.com/dune.mp4", popular: true, thumbnail: "", views: 9800 },
  { id: 3, title: "Oppenheimer", category: "Drama", year: 2023, url: "https://example.com/opp.mp4", popular: false, thumbnail: "", views: 7200 },
];

const initSeries: Series[] = [
  { id: 1, title: "House of the Dragon", category: "Drama", year: 2022, url: "", popular: true, thumbnail: "", seasons: 2, episodes: 18 },
  { id: 2, title: "The Last of Us", category: "Thriller", year: 2023, url: "", popular: true, thumbnail: "", seasons: 1, episodes: 9 },
  { id: 3, title: "Fallout", category: "Sci-Fi", year: 2024, url: "", popular: false, thumbnail: "", seasons: 1, episodes: 8 },
];

const initEpisodes: Episode[] = [
  { id: 1, seriesId: 1, title: "The Heirs of the Dragon", season: 1, episode: 1, url: "https://example.com/hotd-s1e1.mp4", duration: "1h 06m" },
  { id: 2, seriesId: 1, title: "The Rogue Prince", season: 1, episode: 2, url: "https://example.com/hotd-s1e2.mp4", duration: "58m" },
  { id: 3, seriesId: 2, title: "When You're Lost in the Darkness", season: 1, episode: 1, url: "https://example.com/tlou-s1e1.mp4", duration: "1h 21m" },
];

const initUsers: User[] = [
  { id: 1, name: "Alice Nakamura", email: "alice@email.com", joined: "2024-01-15", plan: "1 Month", planExpiry: "2026-04-15", status: "active" },
  { id: 2, name: "Bob Okello", email: "bob@email.com", joined: "2024-03-20", plan: "1 Week", planExpiry: "2026-03-27", status: "expired" },
  { id: 3, name: "Carol Nambi", email: "carol@email.com", joined: "2025-02-10", plan: null, planExpiry: null, status: "none" },
  { id: 4, name: "David Ssali", email: "david@email.com", joined: "2025-06-01", plan: "1 Month", planExpiry: "2026-07-01", status: "active" },
  { id: 5, name: "Eve Apio", email: "eve@email.com", joined: "2025-08-14", plan: null, planExpiry: null, status: "none" },
  { id: 6, name: "Frank Mugisha", email: "frank@email.com", joined: "2025-11-03", plan: "1 Day", planExpiry: "2026-03-24", status: "active" },
];

const initTransactions: Transaction[] = [
  { id: 1, type: "subscription", amount: 20000, description: "1 Month Plan — Alice Nakamura", date: "2026-03-15", status: "completed" },
  { id: 2, type: "subscription", amount: 5000, description: "1 Week Plan — Bob Okello", date: "2026-03-20", status: "completed" },
  { id: 3, type: "withdrawal", amount: 15000, description: "Withdrawal to 0776123456", date: "2026-03-21", status: "completed", phone: "0776123456" },
  { id: 4, type: "subscription", amount: 20000, description: "1 Month Plan — David Ssali", date: "2026-03-22", status: "completed" },
  { id: 5, type: "subscription", amount: 2000, description: "1 Day Plan — Frank Mugisha", date: "2026-03-23", status: "completed" },
  { id: 6, type: "withdrawal", amount: 10000, description: "Withdrawal to 0701987654", date: "2026-03-23", status: "pending", phone: "0701987654" },
];
...
const navItems: { id: Section; label: string; icon: JSX.Element }[] = [
  ...
  },
  {
    id: "movies",
    label: "Movies",
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18 4l2 4h-3l-2-4h-2l2 4h-3l-2-4H8l2 4H7L5 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V4h-4z"/></svg>,
  },
  {
    id: "series",
    label: "Series",
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M21 3H3c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h5v2h8v-2h5c1.1 0 1.99-.9 1.99-2L23 5c0-1.1-.9-2-2-2zm0 14H3V5h18v12z"/></svg>,
  },
  {
    id: "episodes",
    label: "Episodes",
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-8 12.5v-9l6 4.5-6 4.5z"/></svg>,
  },
  {
    id: "users",
    label: "Users",
  ...
  },
  {
    id: "wallet",
    label: "Wallet",
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M21 18v1c0 1.1-.9 2-2 2H5c-1.11 0-2-.9-2-2V5c0-1.1.89-2 2-2h14c1.1 0 2 .9 2 2v1h-9c-1.11 0-2 .9-2 2v8c0 1.1.89 2 2 2h9zm-9-2h10V8H12v8zm4-2.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>,
  },
];
...
function GradBtn({ children, onClick, small }: { children: React.ReactNode; onClick?: () => void; small?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 font-semibold text-white hover:opacity-90 transition-opacity rounded-lg ${small ? "text-xs px-3 py-1.5" : "text-sm px-4 py-2"}`}
...
function MoviesSection({ movies, setMovies }: { movies: Movie[]; setMovies: (m: Movie[]) => void }) {
  ...
  const save = () => {
    ...
    setMovies([...movies, { id: Date.now(), title: form.title, category: form.category, year: Number(form.year), url: form.url, thumbnail: form.thumbnail, popular: form.popular, views: 0 }]);
    setForm({ title: "", category: "", year: new Date().getFullYear().toString(), url: "", thumbnail: "", popular: false });
    setShowModal(false);
  };

  const togglePopular = (id: number) => setMovies(movies.map(m => m.id === id ? { ...m, popular: !m.popular } : m));
  const remove = (id: number) => setMovies(movies.filter(m => m.id !== id));
  ...
  return (
    <div>
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <div
                ...
                style={{ background: form.popular ? "linear-gradient(90deg,#a855f7,#ec4899)" : "rgba(255,255,255,0.15)" }}
              >
            ...
            <GradBtn onClick={save}>Upload Movie</GradBtn>
          </div>
        </Modal>
      )}
      ...
      <div className="mb-4">
        <Input placeholder="Search movies..." value={search} onChange={setSearch} />
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr style={{ background: "#1a1d24", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              {["Title", "Category", "Year", "Views", "Popular", "Actions"].map(h => (
                <th key={h} className="text-left text-white/50 text-xs font-medium px-4 py-3">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map((m, i) => (
              <tr key={m.id} style={{ background: i % 2 === 0 ? "rgba(255,255,255,0.02)" : "transparent", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td className="px-4 py-3">
                  <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(168,85,247,0.15)", color: "#a855f7" }}>{m.category}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => togglePopular(m.id)} className={`text-xs px-2 py-0.5 rounded-full font-medium transition-colors ${m.popular ? "bg-green-500/20 text-green-400" : "bg-white/10 text-white/40 hover:bg-white/15"}`}>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-white/30 text-sm">No movies found</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function SeriesSection({ series, setSeries }: { series: Series[]; setSeries: (s: Series[]) => void }) {
  const [showModal, setShowModal] = useState(false);
  ...
  const save = () => {
    if (!form.title || !form.category) return;
    setSeries([...series, { id: Date.now(), title: form.title, category: form.category, year: Number(form.year), url: "", thumbnail: form.thumbnail, popular: form.popular, seasons: Number(form.seasons), episodes: 0 }]);
    setForm({ title: "", category: "", year: new Date().getFullYear().toString(), thumbnail: "", popular: false, seasons: "1" });
    setShowModal(false);
  };

  const togglePopular = (id: number) => setSeries(series.map(s => s.id === id ? { ...s, popular: !s.popular } : s));
  const remove = (id: number) => setSeries(series.filter(s => s.id !== id));
  ...
  return (
    <div>
      {showModal && (
        <Modal title="Add Series" onClose={() => setShowModal(false)}>
          <div className="space-y-3">
            ...
            <GradBtn onClick={save}>Add Series</GradBtn>
          </div>
        </Modal>
      )}
      ...
      <div className="mb-4">
        <Input placeholder="Search series..." value={search} onChange={setSearch} />
      </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
        <table className="w-full">
          <thead>
            <tr style={{ background: "#1a1d24", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              {["Title", "Category", "Year", "Seasons", "Episodes", "Popular", "Actions"].map(h => (
                <th key={h} className="text-left text-white/50 text-xs font-medium px-4 py-3">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map((s, i) => (
              <tr key={s.id} style={{ background: i % 2 === 0 ? "rgba(255,255,255,0.02)" : "transparent", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td className="px-4 py-3">
                  <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(236,72,153,0.15)", color: "#ec4899" }}>{s.category}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => togglePopular(s.id)} className={`text-xs px-2 py-0.5 rounded-full font-medium transition-colors ${s.popular ? "bg-green-500/20 text-green-400" : "bg-white/10 text-white/40 hover:bg-white/15"}`}>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-white/30 text-sm">No series found</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function EpisodesSection({ series, episodes, setEpisodes }: { series: Series[]; episodes: Episode[]; setEpisodes: (e: Episode[]) => void }) {
  const [showModal, setShowModal] = useState(false);
  const [filterSeries, setFilterSeries] = useState("");
  const [form, setForm] = useState({ seriesId: "", title: "", season: "1", episode: "1", url: "", duration: "" });

  const selectedSeriesObj = series.find(s => s.id === Number(form.seriesId));
  const seasons = selectedSeriesObj ? Array.from({ length: selectedSeriesObj.seasons }, (_, i) => String(i + 1)) : ["1"];

  const filtered = episodes.filter(e => !filterSeries || e.seriesId === Number(filterSeries));

  const save = () => {
    if (!form.seriesId || !form.title || !form.url) return;
    setEpisodes([...episodes, { id: Date.now(), seriesId: Number(form.seriesId), title: form.title, season: Number(form.season), episode: Number(form.episode), url: form.url, duration: form.duration }]);
    setForm({ seriesId: "", title: "", season: "1", episode: "1", url: "", duration: "" });
    setShowModal(false);
  };

  const remove = (id: number) => setEpisodes(episodes.filter(e => e.id !== id));

  const getSeriesName = (id: number) => series.find(s => s.id === id)?.title ?? "Unknown";
    ...
    <div>
      {showModal && (
        <Modal title="Add Episode" onClose={() => setShowModal(false)}>
          <div className="space-y-3">
            <Select label="Select Series" options={series.map(s => s.title)} value={selectedSeriesObj?.title ?? ""} onChange={v => { const s = series.find(x => x.title === v); setForm({ ...form, seriesId: s ? String(s.id) : "", season: "1" }); }} />
            <div className="grid grid-cols-2 gap-3">
            ...
            <GradBtn onClick={save}>Add Episode</GradBtn>
          </div>
        </Modal>
      )}
      ...
      <div className="mb-4">
        <Select options={series.map(s => s.title)} value={series.find(s => s.id === Number(filterSeries))?.title ?? ""} onChange={v => { const s = series.find(x => x.title === v); setFilterSeries(s ? String(s.id) : ""); }} />
      </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
        <table className="w-full">
          <thead>
            <tr style={{ background: "#1a1d24", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              {["Series", "S", "E", "Title", "Duration", "Actions"].map(h => (
                <th key={h} className="text-left text-white/50 text-xs font-medium px-4 py-3">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-white/30 text-sm">No episodes found</td></tr>
            )}
          </tbody>
        </table>
      </div>
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        {([["all", "All Users"], ["active", "Active Subscribers"], ["none", "Never Subscribed"]] as const).map(([id, label]) => (
          <button
            key={id}
            onClick={() => setTab(id)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${tab === id ? "text-white" : "text-white/50 hover:text-white/70"}`}
            style={{ background: tab === id ? "linear-gradient(90deg,#a855f7,#ec4899)" : "rgba(255,255,255,0.07)" }}
          >
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
        <table className="w-full">
          <thead>
            <tr style={{ background: "#1a1d24", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              {["Name", "Email", "Joined", "Plan", "Expiry", "Status"].map(h => (
                <th key={h} className="text-left text-white/50 text-xs font-medium px-4 py-3">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-white/30 text-sm">No users found</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function WalletSection({ transactions }: { transactions: Transaction[] }) {
  ...
  const doWithdraw = () => {
    if (!phone || !amount || Number(amount) < 1000) return;
          ...
          {withdrawDone ? (
            <div className="text-center py-6">
              <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3" style={{ background: "rgba(34,197,94,0.15)" }}>
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
                  <path d="M5 13l4 4L19 7" stroke="#22c55e" strokeWidth="2.5" strokeLinecap="round"/>
                </svg>
              </div>
            ...
            <div className="space-y-3">
              ...
              <GradBtn onClick={doWithdraw}>
                ...
                Withdraw UGX {amount ? Number(amount).toLocaleString() : "—"}
              </GradBtn>
            </div>
          )}
        </Modal>
      ...
      <div className="flex gap-2 mb-4">
        {([["all", "All"], ["subscription", "Subscriptions"], ["withdrawal", "Withdrawals"]] as const).map(([id, label]) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filter === id ? "text-white" : "text-white/50 hover:text-white/70"}`}
            style={{ background: filter === id ? "linear-gradient(90deg,#a855f7,#ec4899)" : "rgba(255,255,255,0.07)" }}
          >
            {label}
          </button>
        ))}
          ...
          <tbody>
            {filtered.map((t, i) => (
              <tr key={t.id} style={{ background: i % 2 === 0 ? "rgba(255,255,255,0.02)" : "transparent", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${t.type === "subscription" ? "bg-purple-500/15 text-purple-400" : "bg-orange-500/15 text-orange-400"}`}>
                    {t.type === "subscription" ? "Subscription" : "Withdrawal"}
                  </span>
                </td>
                <td className="px-4 py-3 text-white/70 text-sm">{t.description}</td>
                <td className={`px-4 py-3 text-sm font-bold ${t.type === "withdrawal" ? "text-red-400" : "text-green-400"}`}>
                  {t.type === "withdrawal" ? "-" : "+"}UGX {t.amount.toLocaleString()}
                </td>
                <td className="px-4 py-3 text-white/50 text-sm">{t.date}</td>
                <td className="px-4 py-3">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${t.status === "completed" ? "bg-green-500/15 text-green-400" : t.status === "pending" ? "bg-yellow-500/15 text-yellow-400" : "bg-red-500/15 text-red-400"}`}>
...
export function VJDashboard({ onBack }: { onBack: () => void }) {
  ...
  const renderSection = () => {
    switch (section) {
      ...
      case "movies": return <MoviesSection movies={movies} setMovies={setMovies} />;
      case "series": return <SeriesSection series={series} setSeries={setSeries} />;
      case "episodes": return <EpisodesSection series={series} episodes={episodes} setEpisodes={setEpisodes} />;
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "#0d0f13" }}>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full z-[60] flex flex-col transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
        style={{ width: 220, background: "#101114", borderRight: "1px solid rgba(255,255,255,0.07)" }}
      >
        {/* Back + Brand */}
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:ml-[220px]">
        ...
        <div className="sticky top-0 z-40 flex items-center px-5 h-12" style={{ background: "#101114", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "linear-gradient(135deg,#a855f7,#ec4899)" }}>
            ...
            <span className="text-white/60 text-xs hidden sm:block">VJ Emma</span>
          </div>
        </div>
