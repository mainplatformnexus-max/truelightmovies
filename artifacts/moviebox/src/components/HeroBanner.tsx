    ...
    <div className="relative w-full overflow-hidden bg-[#1a1c24]" style={{ aspectRatio: "16/5.5" }}>
      {movies.map((m, i) => (
        ...
        >
          {!imgErrors[m.id] ? (
            ...
            <div className="w-full h-full" style={{ background: `linear-gradient(135deg, #1a2a3a ${i * 30}%, #0a1520 100%)` }} />
          )}
      ...
      <div className="absolute bottom-4 right-4 hidden md:flex items-center gap-2">
        {movies.map((m, i) => (
          ...
          >
            <img src={m.backdrop} alt={m.title} className="w-full h-full object-cover" onError={() => {}} />
          </button>
        ))}
      </div>
    </div>
